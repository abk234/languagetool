#!/usr/bin/env bash
# Sync this fork with languagetool-org/languagetool (fetch-only upstream; push only to origin).
set -euo pipefail
...
UPSTREAM_URL="${UPSTREAM_URL:-git@github.com:languagetool-org/languagetool.git}"
# Repo is multi-GB; keep sync shallow unless you unshallow intentionally.
FETCH_DEPTH="${FETCH_DEPTH:-5}"
...
ensure_remotes() {
  ...
  # Never allow pushes to the original LanguageTool repo.
...
fetch_upstream() {
  info "fetching $UPSTREAM_REMOTE/$UPSTREAM_BRANCH (depth=$FETCH_DEPTH)"
  git fetch --depth "$FETCH_DEPTH" "$UPSTREAM_REMOTE" "$UPSTREAM_BRANCH"
}
...
cmd_status() {
  ...
  if [[ "$behind" =~ ^[0-9]+$ ]] && [[ "$behind" -gt 0 ]]; then
    ...
    echo "Already up to date with $upstream_ref (or shallow history limits comparison)."
  fi
}
...
cmd_push() {
  ...
  info "done (nothing was pushed to LanguageTool upstream)"
}
